...
public class Test06_array {
	...
	public static void main(String[] args) {
		...
		//1. 1차원 배열
		int[] kor=new int[3]; //4바이트 * 3 = 12바이트 메ㅗㅁ리 할당
		kor[0]=10;
		kor[1]=20;
		kor[2]=30;
		...
		for(int i=0; i<eng.length; i++) {
			System.out.println(eng[i]);	//10, 20, 30
		}
		
		double[] aver = {1.2, 3.4, 5.6};
		for(int i=0; i<aver.length; i++) {
			System.out.println(aver[i]); //1.2, 3.4, 5.6
		}
		
		char[] ch= {'H','e','l','l','o'};
		for(int i=0; i<ch.length; i++) {
			System.out.println(ch[i]);	//H, e, l, l, o
		}
		
		String[] name= {"개나리","진달래","무궁화"};
		for(int i=0; i<name.length; i++) {
			System.out.println(name[i]); //개나리, 진달래, 무궁화
		}
